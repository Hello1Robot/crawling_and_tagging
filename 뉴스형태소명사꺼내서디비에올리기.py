...
# MySQL Connection 연결
conn = pymysql.connect(host='localhost', user='root', password='mysql', db='hmje', charset='utf8', port=5433) # 각자 환경에 맞게 환경설정
...
불용어리스트 = ['여성','지역','남성','사업','국가','기자','게임','뉴스','대표','산업','중국','경제','과학','주석','한국','사냥','기업','문제', '대회','시상','지난해','계획','회의','구조','문화','위원회','운영','서비스','센터','이번','부산','확대','관련',
        ...
        '충북', '영산포', '창경궁','그레이트','청주','잠실', '산둥성','광역시','러시아','아프리카','베스트','중남미'
          ]
        ...
        for article in articles:
            ...
            if len(links) >= 2:
                ...
                try:
                    ...
                    for i, j in tagging:
                        if j == 'NNG' and 5 > len(i) > 1 and i not in 불용어리스트: # 일반 명사만 추출
                            명사리스트[i] += 1
                            test_list[i] += 1
                except:
                    print("데이터 오류")
